...
Memos instance URLs, Memos API tokens, Hypothesis API tokens, and optional Hypothesis usernames are stored locally. They are not sent to Wikinote or stored in Chrome Sync.
...
- **Hypothesis**: annotations associated with the account identified by the token you provide, including the selected quotation, your annotation text, document metadata, tags, timestamps, and links.
...
Wikinote does not synchronize full private Memos notes, API tokens, or source credentials. Chrome processes synced data according to your Chrome account and Google privacy settings. Wikinote does not receive a copy of this synchronized data on its own servers.
...
For Memos, use an HTTPS endpoint whenever possible. If you configure an HTTP endpoint, the connection security is controlled by that endpoint and its network environment.
...
Local settings, caches, and favorites remain in your browser until you remove them through the extension, clear the extension’s storage, or uninstall the extension. Chrome Sync records remain subject to your Chrome Sync settings until they are removed or synchronization is disabled. Source services retain data according to their own policies and account controls.

You can:

- disable any source in the Sources panel;
- remove source credentials from the source settings page;
